...
public class TabPageIndicator extends HorizontalScrollView {
	...
	private Drawable indicator,bottmDrawable;
	private TextDrawable[] drawables;
	public Drawable left_edge;
	public Drawable right_edge;
	...
	public void doString(Context context){
		...
		bottmDrawable = getResources().getDrawable(R.drawable.underline_bg);
		// 左右边界阴影效果
		...
		while (i < tabsContainer.getChildCount()) {
			if (i < currentPosition - 1 || i > currentPosition + 1) {
				...
				if (category_text != null) {
					...
					bottmDrawable.setBounds(left,textDrawable.getIntrinsicHeight() + top+6, textDrawable.getIntrinsicWidth() + left, textDrawable.getIntrinsicHeight() + top+19);
					bottmDrawable.draw(canvas);
